...
class CommandEvent(GameEvent):
    ...
    def __str__(self):
        string = self._str_prefix() + f" {self.player} "
        if self.has_ability:
            string += colored(f"Ability ({self.ability_id:X})","green")
            if self.ability:
                string += colored(f" - {self.ability.name}({self.ability.id})","green")
            else:
...
class SelectionEvent(GameEvent):
    """
    Selection events are generated whenever the active selection of the
...
class SetControlGroupEvent(ControlGroupEvent):
    ...
    def __str__(self):
        return colored(self._str_prefix() + f" {self.playerName} CG {self.control_group} Set","magenta")
...
class AddToControlGroupEvent(SetControlGroupEvent):
    ...
    def __str__(self):
        return colored(self._str_prefix()+ f" {self.playerName} CG {self.control_group} Add","magenta")
...
class GetControlGroupEvent(ControlGroupEvent):
    ...
    def __str__(self):
        return colored(self._str_prefix() + f" {self.playerName} CG {self.control_group} Get","magenta")
